...
// Get beneficiary ID for logging
$ben_id = 0;
if ($lease_id > 0) {
    $q = "SELECT beneficiary_id FROM rl_lease WHERE rl_lease_id = ? LIMIT 1";
...
// Insert record
$sql = "INSERT INTO rl_field_visits 
        (lease_id, `date`, officers_visited, visite_status, status, recodrd_on)
        VALUES (?, ?, ?, ?, 1, NOW())";
