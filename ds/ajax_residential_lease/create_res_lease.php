...
/**
 * Generate lease schedules for residential lease (no revision)
 * Premium is only in the first year if first lease
 */
function generateRLSchedules($lease_id, $initial_rent, $premium, $start_date, $duration_years = 30, $penalty_rate = 10.0)
{
    global $con;
...
if ($stmt = $con->prepare($sql)) {
    ...
    if ($stmt->execute()) {
        ...
        // Generate schedules for 30 years
        try {
            generateRLSchedules($newId, $initial_annual_rent, $premium, $start_date, 30, $penalty_rate);
        } catch (Exception $e) {
            // Log error but don't fail the lease creation
            error_log("Schedule generation error for lease $newId: " . $e->getMessage());
        }
        
        // Calculate initial penalties
        try {
            $_REQUEST['lease_id'] = $newId;
