...
// Permission check: use same permission id 22
if (!hasPermission(22)){
...
if ($stmt = mysqli_prepare($con, 'UPDATE rl_land_files SET status=0 WHERE id=?')){
  $file_url = null;
  $ben_id = null;
  if ($stmtF = mysqli_prepare($con, 'SELECT file_url,ben_id FROM rl_land_files WHERE id=? LIMIT 1')){
  ...
  if (!empty($file_url)){
    ...
    $filesRoot = realpath(dirname(__DIR__,2) . DIRECTORY_SEPARATOR . 'resedancial_files');
