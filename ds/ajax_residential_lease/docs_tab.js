(function(){
  ...
  function wireDocsHandlers(container, md5){
    container.querySelectorAll('.docs-upload').forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        fetch('ajax_residential_lease/upload_document.php', { method:'POST', body: fd })
    ...
    container.querySelectorAll('.docs-del').forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        Swal.fire({ icon:'warning', title:'Delete file?', showCancelButton:true }).then(function(res){
          ...
          fetch('ajax_residential_lease/delete_document.php', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: 'id='+encodeURIComponent(id) })
            .then(function(r){ return r.json(); })
            .then(function(resp){
              if (resp && resp.success){
    ...
    container.querySelectorAll('.docs-save-all').forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        fd.set('md5', md5);
        fetch('ajax_residential_lease/update_document_meta.php', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: fd.toString() })
          .then(function(r){ return r.json(); })
          .then(function(resp){
            if (resp && resp.success){
  ...
  function refreshSingleDocRow(container, md5, docType){
    var url = 'ajax_residential_lease/list_documents.php?id=' + encodeURIComponent(md5) +
  ...
  function loadDocsOnce(md5){
    ...
    fetch('ajax_residential_lease/list_documents.php?id=' + encodeURIComponent(md5) + '&_ts=' + Date.now())
      .then(r => r.json())
      .then(resp => {
        if (!resp || !resp.success){
