...
$payments = [];
if ($lease && $lease['rl_lease_id']) {
    if ($stP = mysqli_prepare($con, 'SELECT * FROM rl_lease_payments WHERE lease_id=? AND status=1 ORDER BY payment_date, payment_id')) {
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div>
           ...
           $token = function_exists('encrypt_id') 
                    ...
                    : $lease['rl_lease_id']; ?>
            <a class="btn btn-outline-primary btn-sm" href="ajax_residential_lease/print_schedule_payment.php?token=<?= urlencode($token) ?>"
                target="_blank">
    ...
    <div class="card-block" style="padding: 1rem;">
        ...
        <?php
            ...
            // Residential lease always shows premium (first lease)
            $showPremiumCols = ($lease && isset($lease['is_it_first_ease']) && $lease['is_it_first_ease'] == 1);
