...
$types = [];
$sqlT = 'SELECT doc_type_id, doc_name, doc_group, order_no, approval_required, status, print_url FROM rl_land_document_type WHERE status=1 ORDER BY order_no ASC, doc_name ASC';
...
$files = [];
if ($stmtF = mysqli_prepare($con, 'SELECT id, ben_id, file_type, file_url, status, submitted_date, received_date, referance_no, approval_status FROM rl_land_files WHERE status=1 AND ben_id=?')){
