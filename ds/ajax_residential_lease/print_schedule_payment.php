...
// Payments (status=1)
$payments = [];
if ($stP = $con->prepare('SELECT * FROM rl_lease_payments WHERE lease_id=? AND status=1 ORDER BY payment_date, payment_id')) {
...
$lease_start_date = $lease['lease_start_date'] ?? ($lease['start_date'] ?? '');
...
// Residential lease shows premium only for first ease
$showPremiumCols = ($lease && isset($lease['is_it_first_ease']) && (int)$lease['is_it_first_ease'] === 1);
