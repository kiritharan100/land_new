...
if ($id === 0){
  ...
  $ben_id = 0;
  if ($md5 !== ''){
    if ($stmtB = mysqli_prepare($con, 'SELECT rl_ben_id FROM rl_beneficiaries WHERE md5_ben_id=? LIMIT 1')){
      mysqli_stmt_bind_param($stmtB, 's', $md5);
      mysqli_stmt_execute($stmtB);
      $resB = mysqli_stmt_get_result($stmtB);
      if ($resB && ($rowB = mysqli_fetch_assoc($resB))) { $ben_id = (int)$rowB['rl_ben_id']; }
  ...
  if ($ben_id > 0){
    ...
    if ($id === 0){
      if ($stmtI = mysqli_prepare($con, "INSERT INTO rl_land_files (ben_id, file_type, file_url, status, approval_status, submitted_date, received_date, referance_no) VALUES (?, ?, '', 1, ?, ?, ?, ?)")){
...
if ($stmt = mysqli_prepare($con, 'UPDATE rl_land_files SET approval_status=?, submitted_date=?, received_date=?, referance_no=? WHERE id=?')){
