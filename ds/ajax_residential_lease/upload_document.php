...
// Permission: reuse permission 23 (document upload)
if (!hasPermission(23)){
...
if (!$ben_id && !empty($_POST['id'])){
  ...
  if ($stmt = mysqli_prepare($con, 'SELECT rl_ben_id FROM rl_beneficiaries WHERE md5_ben_id=? LIMIT 1')){
    ...
    if ($res && ($row = mysqli_fetch_assoc($res))) { $ben_id = (int)$row['rl_ben_id']; }
...
$approval_required = 0;
if ($stmtT = mysqli_prepare($con, 'SELECT approval_required FROM rl_land_document_type WHERE doc_type_id=? LIMIT 1')){
...
$md5_ben_id = '';
if ($stmtB = mysqli_prepare($con, 'SELECT md5_ben_id FROM rl_beneficiaries WHERE rl_ben_id=? LIMIT 1')){
...
$baseDir = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'resedancial_files' . DIRECTORY_SEPARATOR . $location_id;
...
$file_url = '/resedancial_files/' . $location_id . '/' . $filename;

$existing_id = null;
if ($stmtE = mysqli_prepare($con, 'SELECT id FROM rl_land_files WHERE ben_id=? AND file_type=? AND status=1 LIMIT 1')){
...
if ($existing_id){
  if ($stmtU = mysqli_prepare($con, 'UPDATE rl_land_files SET file_url=?, approval_status=?, submitted_date=?, received_date=?, referance_no=? WHERE id=?')){
  ...
  if ($stmtI = mysqli_prepare($con, 'INSERT INTO rl_land_files (ben_id, file_type, file_url, status, submitted_date, received_date, approval_status, referance_no) VALUES (?, ?, ?, 1, ?, ?, ?, ?)')){
    ...
    if ($ok && $newId > 0) {
      UserLog("2", "RL Document Uploaded", "Ben ID=$ben_id | File Type=$doc_type_id | File URL=$file_url", $ben_id);
