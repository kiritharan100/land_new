...
$sql = "SELECT ul.log_date, ul.action, ul.detail, ul.usr_id, ul.lease_type, u.username, u.i_name
        ...
        WHERE ul.ben_id = ? AND (ul.lease_type = 'LTL' OR ul.lease_type = '' OR ul.lease_type IS NULL)
...
<div class="table-responsive">
    <table class="table table-bordered table-sm">
        ...
        <tbody>
            <?php if (empty($logs)): ?>
            <tr><td colspan="4" class="text-center text-muted">No logs found for the selected range.</td></tr>
            <?php else: foreach ($logs as $log): ?>
            <tr>
                <td><?= htmlspecialchars($log['log_date'] ?? '') ?></td>
                <td><?= htmlspecialchars($log['i_name'] ?? $log['username'] ?? ('User #' . ($log['usr_id'] ?? ''))) ?></td>
