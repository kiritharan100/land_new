...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div>
            ...
            <button type="button" class="btn btn-success btn-sm" id="ltl-record-payment-btn"><i class="fa fa-plus"></i>
    ...
    <div class="card-block" style="padding: 1rem;">
      ...
      ?>
        ...
        .ltl-payments-table th.col-date,
        .ltl-payments-table td.col-date {
            width: 9.5rem;
        }

        .ltl-payments-table th.col-amt,
        .ltl-payments-table td.col-amt {
            width: 10rem;
            text-align: right;
        }

        .ltl-payments-table tr.cancelled-payment-row {
            background: #fde2e2;
        }

        .ltl-payments-table tr.cancelled-payment-row td {
            color: #842029;
        }
        ...
        (function() {
            ...
            if (btn) {
                btn.addEventListener('click', function() {
                    ...
                    var url =
                        'ltl_ajax/payment_form.php?lease_id=<?= (int)($lease['lease_id'] ?? 0) ?>&_ts=' +
                        Date.now();
                    fetch(url)
                        .then(function(r) {
                            return r.text();
                                    ...
                                    calcBtn.addEventListener('click', function() {
                                        ...
                                        if (!paymentDate) {
                                            Swal.fire('Validation',
                                                'Please select payment date first',
                                                'warning');
                                            return;
                                        }
                                        ...
                                        var calcUrl =
                                            'ajax/calculate_outstanding.php?lease_id=<?= (int)($lease['lease_id'] ?? 0) ?>&payment_date=' +
                                                ...
                                                if (resp && resp.success) {
                                                    if (resultEl) {
                                                        resultEl.innerHTML =
                                                            ...
                                                            'Rent: LKR ' + Number(resp
                                                                .outstanding_rent)
                                                            .toLocaleString('en-US', {
                                                                minimumFractionDigits: 2
                                                            }) + '<br>' +
                                                            'Penalty: LKR ' + Number(resp
                                                                .penalty_amount)
                                                            .toLocaleString('en-US', {
                                                                minimumFractionDigits: 2
                                                            }) + '<br>' +
                                                            '<strong>Total Due: LKR ' +
                                                            Number(resp.total_due)
                                                            .toLocaleString('en-US', {
                                                                minimumFractionDigits: 2
                                                            }) + '</strong>';
                                                    }
                                                    var amountEl = body.querySelector(
                                                        '#amount');
                                                    if (amountEl) {
                                                        amountEl.value = resp.total_due;
                                                    }
                                                } else {
                                                    if (resultEl) {
                                                        resultEl.textContent =
                                                            'Error calculating outstanding amount';
                                                    }
                                                }
                                            })
                                            .catch(function() {
                                ...
                                if (form) {
                                    form.addEventListener('submit', function(ev) {
                                        ...
                                        fetch(form.getAttribute('action') ||
                                                'ajax/record_payment_simple.php', {
                                                    method: 'POST',
                                                    headers: {
                                            ...
                                            .then(function(txt) {
                                                ...
                                                try {
                                                    ...
                                                    Swal.fire({
                                                        title: 'Error!',
                                                        text: 'Server response was not JSON: ' +
                                                            (txt ? txt.substring(0,
                                                                    300) :
                                                                'Empty response'),
                                                        icon: 'error'
                        ...
                        .catch(function() {
                            body.innerHTML =
                                '<div class="text-danger">Failed to load payment form.</div>';
                        });
                    if (window.jQuery) {
                        jQuery('#ltl-payment-modal').modal('show');
                    }
                });
            }
            ...
            document.querySelectorAll('.ltl-cancel-payment-btn').forEach(function(b) {
                b.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        html: 'Receipt: <strong>' + receipt + '</strong><br>Date: ' +
                            pdate + '<br>Amount: LKR ' + Number(amount).toLocaleString(
                                'en-US', {
                                    minimumFractionDigits: 2
                                }),
                        icon: 'warning',
                        ...
                        fetch('ajax/cancel_payment.php', {
                            ...
                            .then(function(resp) {
                                Swal.close();
                                if (resp && resp.success) {
                                    Swal.fire('Cancelled', resp.message ||
                                        'Payment cancelled and penalties recalculated',
                                        'success').then(function() {
                                        if (typeof window.dispatchEvent ===
                                            'function') {
