...
try{
    ...
    if ($rsOld = mysqli_query(
        ...
        "SELECT valuation_amount, start_date, annual_rent_percentage, end_date,
                ...
                first_lease, last_lease_annual_value
...
detectChange("first_lease",            $oldLease['first_lease'] ?? "",      $first_lease, $changes);
detectChange("last_lease_annual_value", $oldLease['last_lease_annual_value'] ?? "", $last_lease_annual_value, $changes);
   ...
   $effective_pct = $annual_rent_percentage; 
    ...
    if ($oldLease){
        ...
        // }
              ...
              (int)($oldLease['first_lease'] ?? 1) !== $first_lease ||
              round(floatval($oldLease['last_lease_annual_value'] ?? 0),2) != round($last_lease_annual_value,2)
          ) {
              $need_rebuild_and_replay = true;
          }
    ...
    if ($stmt = mysqli_prepare($con, $sql)){
        mysqli_stmt_bind_param(
            $stmt,
            'iisssdsssddidssiidsii',
            $beneficiary_id, $location_id, $lease_number, $file_number,$initial_annual_rent,
            ...
            $first_lease, $last_lease_annual_value, $name_of_the_project, $uid, $lease_id
        );
    ...
    // ----------------------------------------------------
    // Always regenerate schedules with current lease values, then replay payments if any.
    ...
    // Recalculate penalties on the freshly generated schedules so replay uses current penalty balances
    if (!$skip_penalty) {
        try {
    ...
    if ($payments_count > 0) {
        if (!reapplyPaymentsOnExistingSchedules($con, $lease_id)) {
            throw new Exception('Failed to reapply payments on regenerated schedules.');
        }
...
function reapplyPaymentsOnExistingSchedules(mysqli $con, int $lease_id): bool
{
    ...
    // Reset payment summaries and detach schedule for replay
    if ($stResetPay = mysqli_prepare($con, "UPDATE lease_payments SET schedule_id=NULL, rent_paid=0, panalty_paid=0, premium_paid=0, discount_apply=0, current_year_payment=0 WHERE lease_id=? AND status=1")) {
        mysqli_stmt_bind_param($stResetPay, 'i', $lease_id);
        if (!mysqli_stmt_execute($stResetPay)) {
            $con->rollback();
    ...
    // Remove all payment details for this lease before replay
    if ($stDelDetailAll = mysqli_prepare($con, "DELETE FROM lease_payments_detail WHERE payment_id IN (SELECT payment_id FROM lease_payments WHERE lease_id=? )")) {
        mysqli_stmt_bind_param($stDelDetailAll, 'i', $lease_id);
        if (!mysqli_stmt_execute($stDelDetailAll)) {
            $con->rollback();
...
// Lease Shedule  generate each year
